...
{
    pthread_mutex_lock(&print_mutex);
    const char *state_type = park_open ? "SYSTEM STATE =>" : "FINAL STATISTICS:";
    printf("[Monitor] %s\n", state_type);
    ...
    for (int i = 0; i < sim.C; i++) {
        ...
        printf("Car status %d %s (%d/%d Passengers)\n",
...
// helper funciton for thread safe dequeueing 
{
    ...
    queue[size - 1] = -1;
    pthread_mutex_unlock(&queue_type);  // ← moved above return
    return removed;
}
