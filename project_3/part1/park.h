...
/* Timing */
extern time_t park_start;          /* set once in main before threads launch */
 
/* Park open flag */
extern volatile int park_open;
 
/* Ticket queue length (how many passengers waiting for a ticket) */
extern int ticket_queue_len;
 
/* Ride queue length (how many passengers waiting to board) */
...
extern pthread_mutex_t state_mutex;    /* protects all shared state below      */
...
extern int passengers_unboarded;   /* count of passengers that have unboarded */
 
/* Global simulation params (set in main, read-only after that) */
extern SimParams g;
